...
let to_string =
  ...
  | PtrAdd (dest, arg1, arg2) ->
      sprintf "%s ptradd %s %s" (dest_to_string dest) arg1 arg2
...
let of_json json =
  ...
  | `Null -> (
      ...
      in
      let labels () =
        json |> member "labels" |> Common.to_list_nonnull
      ...
      | "const" ->
          let const =
            ...
            | t ->
                failwithf "invalid operand to const: %s" (Bril_type.to_string t)
                  ()
          in
          Const (dest (), const)
      ...
      | "call" ->
          Call
            ( (if Common.has_key json "dest" then Some (dest ()) else None),
              json |> member "funcs" |> Common.to_list_nonnull |> List.hd_exn
              |> to_string,
              args () )
...
let to_json =
  ...
  | Const (dest, const) ->
      `Assoc
        ([
           ("op", `String "const");
           ("value", match const with Int i -> `Int i | Bool b -> `Bool b);
         ]
        @ dest_to_json dest)
  | Binary (dest, op, arg1, arg2) ->
      `Assoc
  ...
  | Ret arg ->
      `Assoc
        [
          ("op", `String "ret");
          ( "args",
            Option.value_map arg ~default:`Null ~f:(fun a ->
                `List [ `String a ]) );
        ]
  | Print args ->
      `Assoc
  ...
  | Alloc (dest, arg) ->
      `Assoc
        ([ ("op", `String "alloc"); ("args", `List [ `String arg ]) ]
        @ dest_to_json dest)
  | Free arg ->
      `Assoc [ ("op", `String "free"); ("args", `List [ `String arg ]) ]
  | Store (arg1, arg2) ->
      `Assoc
        [
          ("op", `String "store"); ("args", `List [ `String arg1; `String arg2 ]);
        ]
  | Load (dest, arg) ->
      `Assoc
        ([ ("op", `String "load"); ("args", `List [ `String arg ]) ]
        @ dest_to_json dest)
  | PtrAdd (dest, arg1, arg2) ->
      `Assoc
        ([
